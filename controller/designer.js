...
exports.updateQuotaStartState = async (req, res) => {
	...
	let designer_start_finish = Date.now() / 1000 |0;
	let data = {designer_start_finish};
	await mysql.query("UPDATE t_projects_quota SET ? WHERE ?", [data, {id}]);
	return res.json({designer_start_finish});
};
//监理完成验收
exports.updateQuotaEndState = async (req, res) => {
	...
	let designer_end_finish = Date.now() / 1000 |0;
	let data = {designer_end_finish};
	await mysql.query("UPDATE t_projects_quota SET ? WHERE ?", [data, {id}]);
	return res.json({designer_end_finish});
};
